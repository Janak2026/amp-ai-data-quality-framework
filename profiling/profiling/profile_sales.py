...
# DBTITLE 1,Load sales_silver
# Load SILVER Sales Table
...
numeric_cols = [f.name for f in df_sales.schema.fields if isinstance(f.dataType, (T.IntegerType, T.LongType, T.DoubleType, T.FloatType))]
string_cols = [f.name for f in df_sales.schema.fields if isinstance(f.dataType, T.StringType)]
date_cols = [f.name for f in df_sales.schema.fields if isinstance(f.dataType, T.DateType)]
...
# DBTITLE 1,Schema Drift Check
# Current schema snapshot
current_schema = {f.name: str(f.dataType) for f in df_sales.schema.fields}

# For now, baseline = current (first run)
...
# DBTITLE 1,Null Anomaly Check
null_anomalies = {}
total_rows = df_sales.count()

for col_name in df_sales.columns:
    null_count = df_sales.filter(F.col(col_name).isNull()).count()
...
# DBTITLE 1,Outlier Detection (quantity, amount)
outlier_report = {}

for col_name in numeric_cols:
    stats_row = df_sales.select(
    ...
    df_outliers = df_sales.filter((F.col(col_name) - mean_val) / std_val > 3)
...
# DBTITLE 1,AI Summary
def generate_ai_summary(stats, drift, nulls, outliers):
    summary = []

    if drift["missing_columns"] or drift["new_columns"] or drift["type_mismatches"]:
        summary.append("⚠ Schema drift detected in sales_silver. Please review the changes.")
    else:
        summary.append("✔ No schema drift detected in sales_silver. Schema matches baseline.")

    high_null_columns = [col for col, v in nulls.items() if v["null_percentage"] > 20]
    if high_null_columns:
        summary.append(f"⚠ High null percentages in: {', '.join(high_null_columns)}.")
    else:
        summary.append("✔ No significant null anomalies detected in sales_silver.")
    ...
    if outlier_cols:
        ...
        summary.append("✔ No numeric outliers detected in sales_silver.")

    summary.append("✔ Dataset looks healthy and ready for GOLD aggregations.")
    return "\n".join(summary)
